...
def test_execute_pricing_falls_back_to_snapshot_when_rtd_option_quote_is_stale(tmp_path):
    app_db = tmp_path / "app.db"
    _create_controlled_app_db(app_db)
    ...
    with sqlite3.connect(str(app_db)) as conn:
        conn.execute(
            ...
            SET ultimo_preco = ?, bid = ?, ask = ?, updated_at = ?
            WHERE codigo_opcao = ?
            """,
            (9.99, 9.50, 10.50, "2000-01-01 00:00:00", "ABCD11"),
        )
        conn.commit()
    ...
    # Com quote RTD presente e preço utilizável, mas vencida/stale,
    # o preço efetivo deve voltar para o snapshot original da perna.
    assert leg["symbol"] == "ABCD11"
    assert leg["asset"] == "ABCD11"
    assert leg["price"] == 5.55
    assert leg["premium"] == 5.55
    assert leg["price_source"] == "snapshot"
    assert leg["price_resolution_status"] == "stale_rtd_quote"
    assert leg["rtd_quote_found"] is True
    assert leg["rtd_validation_status"] == "warn"
    assert "vencida" in leg["rtd_validation_message"]

    assert leg["rtd_quote_codigo_opcao"] == "ABCD11"
    assert leg["rtd_quote_ativo_base"] == "ABCD"
    assert leg["rtd_price_source"] == "rtd_option_quotes"
    assert leg["rtd_price_updated_at"] == "2000-01-01 00:00:00"
    assert leg["rtd_price_created_at"] == "2026-06-15T10:01:00"
    ...
    assert persisted_leg["premium"] == 5.55
    assert persisted_leg["price_source"] == "snapshot"
    assert persisted_leg["price_resolution_status"] == "stale_rtd_quote"
    assert persisted_leg["rtd_quote_found"] is True
    assert persisted_leg["rtd_validation_status"] == "warn"
    assert "vencida" in persisted_leg["rtd_validation_message"]
    ...
    assert persisted_leg["rtd_price_updated_at"] == "2000-01-01 00:00:00"
