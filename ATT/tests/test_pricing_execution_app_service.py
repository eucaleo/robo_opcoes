...
class FakePricingExecutionOrchestrationService:
    def __init__(self, response):
        self.response = response
        self.calls = []

    def execute_and_persist(self, structure_id: int, reference_date: str | None = None):
...
def _make_service(response, query_service=None):
    return PricingExecutionAppService(
        pricing_execution_orchestration_service=FakePricingExecutionOrchestrationService(response),
...
def test_execute_pricing_returns_persisted_record_when_present():
    orchestration = FakePricingExecutionOrchestrationService(response={
        "persisted": {
            "record": {
                "id": 123,
                "structure_id": 10,
    ...
    service = PricingExecutionAppService(
        pricing_execution_orchestration_service=orchestration,
    ...
    assert orchestration.calls == [{"structure_id": 10, "reference_date": "2026-05-16"}]
...
def test_execute_pricing_returns_raw_response_when_persisted_record_is_missing():
    ...
    orchestration = FakePricingExecutionOrchestrationService(response=raw_response)
    ...
    assert orchestration.calls == [{"structure_id": 11, "reference_date": "2026-05-16"}]
...
def test_execute_pricing_accepts_none_reference_date():
    orchestration = FakePricingExecutionOrchestrationService(
    ...
    assert result == {"id": 55}
    assert orchestration.calls == [{"structure_id": 10, "reference_date": None}]


def test_execute_pricing_raises_value_error_when_orchestration_returns_error_status():
    orchestration = FakePricingExecutionOrchestrationService(
        response={"status": "error", "error_message": "pricing failed"}
    )
    ...
    try:
        service.execute_pricing(structure_id=10, reference_date="2026-05-16")
        assert False, "expected ValueError"
    except ValueError as exc:
        assert str(exc) == "pricing failed"


def test_legacy_canonical_facade_is_ignored_by_execution_flow():
    facade = FakeCanonicalPricingFacade(response={
        "persisted": {"record": {"id": 999}}
    })
    orchestration = FakePricingExecutionOrchestrationService(
        response={"persisted": {"record": {"id": 123}}}
    )
