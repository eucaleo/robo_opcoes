...
# === Cria o localizador de bancos ===
cat > scripts/db_locator.py <<'PY'
#!/usr/bin/env python3
"""
...
class DatabaseLocator:
    ...
    def scan_for_databases(self, max_depth=3):
        ...
        print(f"🔍 Escaneando por bancos SQLite em: {self.repo_root}")
    ...
    def save_report(self, report, output_file="ATT/reports/database_scan.json"):
        ...
        print(f"💾 Relatório salvo em: {output_path.relative_to(self.repo_root)}")
    
    def print_summary(self, report):
        """Imprime resumo visual"""
        print("="*60)
        print("📊 RESUMO DO SCAN DE BANCOS")
        print("="*60)
        ...
        print(f"✅ Acessíveis: {accessible_count}")
        print(f"❌ Com problemas: {len(report['databases']) - accessible_count}")
        print()
        ...
        for expected in expected_dbs:
            ...
            if matches:
                ...
                print(f"✅ {expected}: {db['relative_path']}")
            else:
                found_expected[expected] = None
                print(f"❌ {expected}: NÃO ENCONTRADO")
        
        print()
        print("🎯 CONFIGURAÇÃO SUGERIDA PARA PREFLIGHT:")
...
# === Cria wrapper para fácil execução ===
cat > scripts/find_dbs.sh <<'BASH'
#!/bin/bash
# Wrapper para localizar bancos de dados
set -e
...
echo "🔍 Localizando bancos de dados..."
"$PYTHON_BIN" scripts/db_locator.py --save-report
BASH

# === Cria documentação ===
cat > docs/DATABASE_LOCATOR.md <<'DOC'
# Database Locator & Scanner
