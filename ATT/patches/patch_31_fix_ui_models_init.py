...
Critérios de aceite:
  ...
  ✅ UI/models/__ini__.py NÃO existe
  ✅ import UI.models funciona sem erro
  ✅ Backup do estado salvo em BAK/
"""
...
if INIT_OK.exists():
    print(f"  ✅  {INIT_OK.relative_to(RAIZ)} existe")
else:
    print(f"  ❌  {INIT_OK.relative_to(RAIZ)} NÃO encontrado")
    ...
    if INIT_OK.exists():
        print(f"  ✅  Criado: {INIT_OK.relative_to(RAIZ)}")
    else:
...
if INIT_TYPO.exists():
    ...
    print(f"  💾  Backup salvo: {bak_dest.relative_to(RAIZ)}")
    INIT_TYPO.unlink()
    if not INIT_TYPO.exists():
        print(f"  ✅  Typo removido: {INIT_TYPO.relative_to(RAIZ)}")
    else:
        erros.append("__ini__.py não pôde ser removido")
else:
    print(f"  ✅  Typo __ini__.py já foi removido (correção manual aplicada)")
...
try:
    ...
    print(f"  ✅  import UI.models OK — {mod.__file__}")
except Exception as e:
    print(f"  ❌  import UI.models FALHOU: {e}")
    erros.append(f"import UI.models: {e}")

# ── 4. Resultado final ────────────────────────────────────────────────────
print("\n" + "=" * 55)
if erros:
    print(f"  ❌  patch_31 FALHOU — {len(erros)} erro(s):")
    for e in erros:
        print(f"     • {e}")
    sys.exit(1)

print("  ✅  patch_31 CONCLUÍDO com sucesso")
