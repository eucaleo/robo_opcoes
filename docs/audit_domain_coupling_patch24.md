...
|----------|-------|----------|
| CRÍTICO  | [ERRO]    | Bloqueia o desacoplamento canônico; deve ser removido antes do merge |
| MODERADO | [PARCIAL]    | Não bloqueia o pipeline canônico, mas cria dívida técnica mensurável |
| BAIXO    | [INFO]    | Aceitável como metadata/rastreabilidade; revisar oportunisticamente |
...
|-------|-----------|-------|---------|
| 207 | [ERRO] CRÍTICO | `compute_decision_for_aba` | Assinatura da função legada |
| 220 | [ERRO] CRÍTICO | `compute_payoff_for_aba` | Import condicional de função inexistente no payoff canônico |
| 220 | [ERRO] CRÍTICO | `read_structure_summary` | Import condicional de função inexistente no payoff canônico |
| 229 | [ERRO] CRÍTICO | `read_structure_summary` | Chamada direta |
| 241 | [ERRO] CRÍTICO | `compute_payoff_for_aba` | Chamada direta |
| 407-419 | [ERRO] CRÍTICO | `get_app_db_connection` + `rtd_analise_robo` | Bloco `__main__` com query direta ao banco legado |

#### Marcadores canônicos presentes [OK]
