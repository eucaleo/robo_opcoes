...
- Pipeline atual: Excel/RTD -> CSV bridge -> dados/app.db -> domínio -> dados/derived.db -> UI
...
## Fase 3 — Estratégia de integração com o domínio

### Decisão adotada
Foi adotado o caminho 3A: o domínio passará a receber DTOs normalizados, em vez de ler diretamente o raw DB legado.

### Objetivo
Desacoplar payoff/decisão da chave legado `aba + timestamp` e tornar o domínio agnóstico em relação à origem dos dados.

### Nova responsabilidade do orquestrador
A camada de orquestração deverá:
- ler a estrutura canônica no banco
- ler o snapshot de mercado no banco/raw
- normalizar os dados
- montar o DTO de cálculo
- chamar o domínio
- persistir os derivados gerados
...
### Regras da fase
- o domínio não deve acessar diretamente tabelas `rtd_*`
- o domínio não deve depender de `aba` como identidade da estrutura
- o domínio não deve depender de `timestamp` como identidade da estrutura
- `timestamp` pertence ao snapshot de mercado
- `aba` pode existir apenas como alias/metadado legado

### Resultado esperado
O pipeline futuro deverá operar no formato:

`Structure + MarketSnapshot -> Domain Calculation -> derived.db`

## Alteração impostante sobre pasta data/ para dados/
não existe pasta data/, ela foi substituida por dados/ por erros de conflito com "data" dos modulos, 
portanto registre isso pra não ocorrer em erros na geração
