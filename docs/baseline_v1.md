...
Converte erro/vazioNone, númerofloat, textostr, datastring ISO.
...
*	Intrínseco:
*	Call: max(STK,0)max(STK,0)
*	Put: max(KST,0)max(KST,0)
*	P&L unitário:
*	Long: intrinsicpremiumintrinsicpremium
*	Short: premiumintrinsicpremiumintrinsic
*	P&L por perna:
PLperna(ST)=QUANTPLunit(ST)PLperna(ST)=QUANTPLunit(ST)
*	P&L da estrutura:
PLestrutura(ST)=PLperna(ST)PLestrutura(ST)=PLperna(ST)
Malha de preços (grid padrão)
*	Smin=0.5SPOTSmin=0.5SPOT
*	Smax=1.5SPOTSmax=1.5SPOT
*	step:
*	step=max(0.01SPOT,0.01)step=max(0.01SPOT,0.01)
