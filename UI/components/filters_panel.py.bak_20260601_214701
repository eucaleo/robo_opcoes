...
class FiltersPanel(ttk.LabelFrame):
    ...
    def _setup_widgets(self):
        ...
        # Linha 2: Estrutura e Decisão
        row2 = ttk.Frame(self)
        row2.pack(fill="x", pady=5)

        ttk.Label(row2, text="Estrutura:", width=10).pack(side="left")
        ...
        self.structure_combo.pack(side="left", padx=(0, 10))
        ...
        self.apply_btn = ttk.Button(
            btn_frame, text="Aplicar", command=self._apply_filters
        )
        self.apply_btn.pack(side="left", padx=(0, 5))

        self.reset_btn = ttk.Button(
            btn_frame, text="Limpar", command=self.reset_filters
        )
        self.reset_btn.pack(side="left")

        # Status
        self.status_label = ttk.Label(self, text="Filtros prontos", foreground="green")
        self.status_label.pack(fill="x", pady=(5, 0))

    def _bind_events(self):
        for w in [self.date_from_entry, self.date_to_entry, self.level_entry, self.dte_entry]:
            w.bind("<Return>", lambda e: self._apply_filters())

        self.structure_combo.bind("<<ComboboxSelected>>", lambda e: self._apply_filters())
    ...
    def _apply_filters(self):
        ...
        if self.structure_var.get().strip():
        ...
        self.status_label.config(
            text=f"Filtros aplicados ({len(filters)} ativos)",
            foreground="blue",
        )
    ...
    def update_structures(self, structures: List[str]):
        """Atualiza lista de estruturas no combo (novo método)."""
        current = self.structure_var.get()
        values = [""] + sorted(structures)
        self.structure_combo.config(values=values)
        if current not in values:
            self.structure_var.set("")

    def update_abas(self, abas: List[str]):
        """Alias de compatibilidade — delega para update_structures()."""
        self.update_structures(abas)
