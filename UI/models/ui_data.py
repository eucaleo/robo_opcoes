...
class UIDataModel:
    ...
    def get_decisions(self, filters: Optional[Dict] = None) -> List[Dict]:
        ...
        select_parts = []
        for alias in ["timestamp", "aba", "decision", "level", "dte_min", "why_json", "pl_atual", "pl_max", "spot_ref"]:
            src = c.get(alias)
        ...
        sql = f"""
            SELECT t.timestamp, t.aba, t.decision, t.level, t.pl_pct_of_max, t.dte_min, 
                   t.why_json, t.pl_atual, t.pl_max, t.spot_ref
            FROM {subq}
            {where_sql}
    ...
    def get_payoff_curve(self, aba: str, timestamp: str) -> List[Dict]:
        # --- payoff cache ---
        try:
            _aba = aba  # nome mais comum
        except NameError:
            _aba = None
        # tenta capturar timestamp se existir na assinatura/variáveis
        _ts = locals().get('timestamp')
        if _ts is None:
            _ts = locals().get('ts')
        # se o método resolve "último ts" internamente, cacheia pelo marcador '__latest__'
        ...
        if hasattr(self, '_payoff_cache') and _cache_key in self._payoff_cache:
            # print('[UI] PAYOFF_CACHE_HIT', _cache_key)
    ...
    def export_to_csv(self, data: List[Dict], filename: str):
        if not data:
            headers = ["timestamp", "aba", "decision", "level", "pl_pct_of_max", "dte_min", "why_json", "pl_atual", "pl_max", "spot_ref"]
    ...
    def _connect_derived_threadsafe(self):
        ...
        # tentar reaproveitar configuração existente
