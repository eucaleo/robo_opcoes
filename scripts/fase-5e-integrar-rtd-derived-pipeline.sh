...
path = Path("scripts/run_derived_pipeline.py")
...
_RTD_METRIC_RE = re.compile(r"^\s*(input_rows|inserted|updated|skipped):\s*(-?\d+)\s*$")
...
def _parse_rtd_pipeline_metrics(output: str) -> dict:
    """Extrai métricas textuais emitidas pelo pipeline RTD restaurado."""
    metrics = {
        "input_rows": None,
        "inserted": 0,
        "updated": 0,
    ...
    for line in (output or "").splitlines():
        match = _RTD_METRIC_RE.match(line)
        if not match:
            continue
...
def _rtd_quotes_updated_count(rtd_result: dict | None) -> int:
    """Retorna o total de cotações efetivamente inseridas/atualizadas."""
    if not rtd_result:
        return 0

    return int(rtd_result.get("inserted") or 0) + int(rtd_result.get("updated") or 0)


def _run_rtd_option_quotes_import(
    repo_root: Path,
    csv_path: str = "dados/RTD_LINKS.csv",
    db_path: str = "dados/derived.db",
) -> dict:
    """
    Executa a cadeia operacional RTD já restaurada contra o derived.db.

    Importante:
    - Usa somente CSV local dados/RTD_LINKS.csv.
    - Não aciona Excel, PowerShell, COM ou refresh RTD ao vivo.
    - Delega importação + auditoria para scripts/run_rtd_option_quotes_pipeline.py.
    """
    repo_root = Path(repo_root)
    pipeline_script = repo_root / "scripts" / "run_rtd_option_quotes_pipeline.py"
    resolved_csv = repo_root / csv_path
    resolved_db = repo_root / db_path
    ...
    if not pipeline_script.exists():
        return {
            ...
            "errors": 1,
            "warnings": 0,
            "message": f"Script RTD não encontrado: {pipeline_script}",
            "input_rows": None,
            "inserted": 0,
            "updated": 0,
            "skipped": 0,
    ...
    if not resolved_csv.exists():
        return {
            ...
            "errors": 1,
            "warnings": 0,
            "message": f"CSV RTD não encontrado: {resolved_csv}",
            "input_rows": None,
            "inserted": 0,
            "updated": 0,
            "skipped": 0,
    ...
    metrics = _parse_rtd_pipeline_metrics(stdout)
...
def _collect_pipeline_summary(rtd_result: dict | None = None) -> dict:
    ...
    Fase 5E:
    - Inclui a quantidade de cotações RTD inseridas/atualizadas.
    - Não aciona Excel.
    - Apenas lê contagens do banco depois que o pipeline RTD CSV já rodou.
    """
    from db.config import connect_derived
    ...
    try:
        ...
        return {
            ...
            "errors": int((rtd_result or {}).get("errors") or 0),
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--skip-rtd",
        action="store_true",
        help="Não importar dados/RTD_LINKS.csv para rtd_option_quotes no derived.db",
    )
    parser.add_argument(
        "--rtd-csv",
        default="dados/RTD_LINKS.csv",
        help="Caminho do CSV RTD usado pelo pipeline derivado",
    )
    ...
    if args.skip_rtd:
        ...
        rtd_result = _run_rtd_option_quotes_import(
            PROJECT_ROOT,
            csv_path=args.rtd_csv,
            db_path="dados/derived.db",
        )

        if int(rtd_result.get("returncode") or 0) != 0:
            print("[ERROR] PIPELINE FALHOU: importação/auditoria RTD falhou")
            if rtd_result.get("message"):
                print(f"[ERROR] {rtd_result.get('message')}")
            summary = _collect_pipeline_summary(rtd_result)
...
cat > ATT/tests/test_run_derived_pipeline_rtd_integration.py <<'PY'
