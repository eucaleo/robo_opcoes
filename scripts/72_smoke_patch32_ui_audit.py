...
def main():
    ...
    if not spec_path.exists():
        print(f"  [FALHOU]  Não encontrado: {spec_path.relative_to(RAIZ)}")
        sys.exit(1)
    ...
    if not md_path.exists():
        print(f"\n  [FALHOU]  Relatório não gerado: {md_path.relative_to(RAIZ)}")
        sys.exit(1)

    print(f"\n  [OK]  Relatório gerado: {md_path.relative_to(RAIZ)}")
    print(f"  {'[OK]' if exit_code == 0 else '[AVISO] '} Exit code patch_32: {exit_code}")
    print("\n  smoke_72 concluído.")
